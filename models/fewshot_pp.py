...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        self.decoder = nn.Sequential(OrderedDict([
            ('Neck', Decoder(1, self.pretrained_path)),]))
...
#             outputs.append(F.interpolate(pred, size=img_size, mode='bilinear'))
            # Upsample to N x (1 + Wa) x H x W with decoder network
            outputs.append(torch.cat([self.decoder(l[:,None,:,:]) for l in pred.transpose(1,0)], dim=1))
