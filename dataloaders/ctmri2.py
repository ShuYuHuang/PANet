...
#         return torch.tensor(x,dtype=torch.float32,device=self.device)
    def getitem(self, idx):
        ...
        if self.mask_type == 'MRI':
            ...
            img_path2 = img_anno_path[1]
            mask_path = img_anno_path[2]
        if self.mask_type == 'CT':
            img_path = img_anno_path[0]
...
#         '''
        if img_path.__contains__('.dcm'):    
            # pydcm read image
            ds = dcmread(img_path)
            image = ds.pixel_array # 調整格式以配合albumentation套件需求
        else:
            raise ValueError(f'img path: {img_path} unknown')
        if self.mask_type == 'MRI': 
            if img_path2.__contains__('.dcm'):    
                # pydcm read image
                ds = dcmread(img_path2)
                image2 = ds.pixel_array # 調整格式以配合albumentation套件需求
            else:
                raise ValueError(f'img path: {img_path} unknown')
            image=((image-image2)/2+127.).astype('uint8')
        else:
